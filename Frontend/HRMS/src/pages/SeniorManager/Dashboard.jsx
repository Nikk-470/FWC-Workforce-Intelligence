...
export default function SeniorManagerDashboard() {
  ...
  const fetchDashboardData = async (targetManagerId) => {
    ...
    try {
      ...
      const empRes = await axios.get("${import.meta.env.VITE_API_BASE_URL}/api/employees", {
        headers: { Authorization: `Bearer ${token}` }
      });
      setEmployees(empRes.data);

      // 2. Grab your custom managed database teams
      const teamRes = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/teams?managerId=${targetManagerId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleUpdateManagerProfile = async (e) => {
    ...
    try {
      const token = localStorage.getItem("fwc_token");
      await axios.put(`${import.meta.env.VITE_API_BASE_URL}/api/employees/${managerUser._id}`, {
...
const handleCreateTeam = async (e) => {
  ...
  const handleAddMemberToTeam = async (employeeId) => {
    try {
      ...
      const response = await axios.put(`${import.meta.env.VITE_API_BASE_URL}/api/employees/${employeeId}`, {
...
function LeaveTriagePanel() {
  ...
  const handleTriageAction = async (id, actionString) => {
    try {
      const token = localStorage.getItem("fwc_token");
      const res = await axios.patch(`${import.meta.env.VITE_API_BASE_URL}/api/leaves/admin/triage/${id}`, 
        { action: actionString },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (res.data.success) {
...
function ManagerMeetingControlPanel({ managerUser, teams, departmentEmployees }) {
  ...
  const loadManagerRooms = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/meetings/manager/${managerUser._id}`);
